	...
	*/
	...
	_prepareItems(data) {
		...
		const features = {
			...
			actions:	{ prep: this.prepAction.bind(this), filter: item => Boolean(item.data?.activation?.type && item.data.activation.type != "none" && item.data.activation.type != "bonus"), label: game.i18n.localize("DND5E.ActionPl"), items: [] , dataset: {type: "feat"} },
